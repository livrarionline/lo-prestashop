...
class LO extends Module
{
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->trans('Livrari Online');
        $this->description = $this->trans('Ship your goods using "Livrari Online" system');
    ...
    public function install()
    {
        ...
        $config = array(
            ...
            'range_behavior' => 0,
            'is_module' => true,
            'delay' => array(Language::getIsoById(Configuration::get('PS_LANG_DEFAULT')) => $this->trans('Fast carrier: 24h')),
            'id_zone' => 1,
    ...
    public function installDB()
    {
        ...
        $queries[] = 'CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'lo_locker_push` (
    ...
    {
        $national_fields = json_decode(Configuration::get('LO_NATIONAL_FIELDS'), true);
        $lockers_carrier = 0;
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('submitLoModule')) {
                ...
                if (Tools::getIsset('national_field_' . $field)) {
                    foreach (Tools::getValue('national_field_' . $field) as $field_index => $field_val) {
                        ...
                        if ($field == 'serviciu' && !$field_val) {
                            $errors[] = $this->trans('Service name is mandatory.');
                            $values_array[$field][] = "";
                        } elseif ($field == 'serviciuid' && !Validate::isInt($field_val)) {
                            $errors[] = $this->trans('Service ID has to be an integer.');
                            $values_array[$field][] = "";
                        } elseif ($field == 'shippingcompanyid' && !Validate::isInt($field_val)) {
                            $errors[] = $this->trans('Shipping Company ID has to be an integer.');
                            $values_array[$field][] = "";
                        } elseif ($field == 'flatprice' && $field_val && !Validate::isFloat($field_val)) {
                            $errors[] = $this->trans('Flat price has to be a decimal number.');
                            $values_array[$field][] = "";
                        } elseif ($field == 'fixedprice' && $field_val && !Validate::isFloat($field_val)) {
                            $errors[] = $this->trans('Fixed price has to be a decimal number.');
                            $values_array[$field][] = "";
                        } elseif ($field == 'modify_amount' && $field_val && !Validate::isFloat($field_val)) {
                            $errors[] = $this->trans('Modify amount has to be a decimal number.');
                            $values_array[$field][] = "";
                        } elseif ($field == 'freeamount' && $field_val && !Validate::isFloat($field_val)) {
                            $errors[] = $this->trans('Free amount has to be a decimal number.');
                            $values_array[$field][] = "";
                        } else {
            ...
            Configuration::updateValue('LO_NATIONAL_FIELDS', json_encode($final_array));
            ...
            if (!sizeof($errors)) {
                $this->_html .= $this->displayConfirmation($this->trans('Settings saved.'));
            } else {
            ...
            if (Tools::getValue($input['name']) || !isset($input['required']) || !$input['required']) {
                ...
                if (isset($input['validate'])) {
                    if ($input['validate'] == 'isInt' && !Validate::isInt(Tools::getValue($input['name']))) {
                        $ok = false;
                        $errors[] = sprintf($this->trans('%s has to be an integer.'), $input['label']);
                    } elseif ($input['validate'] == 'isEmail' && !Validate::isEmail(Tools::getValue($input['name']))) {
                        $ok = false;
                        $errors[] = sprintf($this->trans('%s is not a valid email address.'), Tools::getValue($input['name']));
                    } elseif ($input['validate'] == 'isPhoneNumber' && !Validate::isPhoneNumber(Tools::getValue($input['name']))) {
                        $ok = false;
                        $errors[] = sprintf($this->trans('%s is not a valid phone number.'), Tools::getValue($input['name']));
                    }
                }
                if ($ok) {
                ...
                $errors[] = sprintf($this->trans('%s is mandatory.'), $input['label']);
                $success = false;
            }
        }

        if ($success) {
            return $this->displayConfirmation($this->trans('Configuration saved.'));
        } else {
            return $this->displayError($this->trans('Please check the errors below.') . "<br /><br />" . implode("<br />", $errors));
        }
    }
    ...
    public function getConfigInputs()
    {
        ...
        return array(
            ...
            array(
                'type' => 'textarea',
                'label' => $this->trans('Security key'),
                'desc' => $this->trans('Also known as rsakey'),
                'name' => 'LO_KEY',
                'required' => true,
            ),
            array(
                ...
                'desc' => $this->trans('On which order state shall we show the module in Admin Order page?'),
                'name' => 'LO_STATUS',
                'options' => array(
                    'query' => $orderStates,
                    'id' => 'id_order_state',
            ...
            array(
                ...
                'desc' => sprintf($this->trans('Get one here: %s'), '<a href="https://developers.google.com/maps/documentation/javascript/get-api-key" target="_blank">https://developers.google.com/maps/documentation/javascript/get-api-key</a>'),
                'name' => 'LO_MAPS_API_KEY',
                'required' => true,
            ),
            array(
                'type' => 'text',
                'label' => $this->trans('Store email'),
                'name' => 'LO_STORE_EMAIL',
                'required' => true,
                'validate' => 'isEmail',
            ),
            array(
                'type' => 'text',
                'label' => $this->trans('Store name'),
                'name' => 'LO_STORE_NAME',
                'required' => true,
            ),
            array(
                'type' => 'text',
                'label' => $this->trans('Store main address'),
                'name' => 'LO_STORE_ADDRESS',
                'required' => true,
            ),
            array(
                'type' => 'text',
                'label' => $this->trans('Store city'),
                'name' => 'LO_STORE_CITY',
                'required' => true,
            ),
            array(
                'type' => 'select',
                'label' => $this->trans('Store state'),
                'name' => 'LO_STORE_STATE',
                'options' => array(
                    'query' => $ro_states,
                    'id' => 'id_state',
            ...
            array(
                'type' => 'text',
                'label' => $this->trans('Store ZIP code'),
                'name' => 'LO_STORE_ZIP',
                'required' => true,
            ),
            array(
                'type' => 'text',
                'label' => $this->trans('Store country'),
    ...
    public function createLOCarrier($config)
    {
        for ($carrier_cont = 1; $carrier_cont <= 5; $carrier_cont++) {
            ...
            foreach ($languages as $language) {
                $carrier->delay[(int)$language['id_lang']] = $this->trans('Fast carrier: 24h');
            }
    ...
    {
        $icon_pp = _PS_BASE_URL_SSL_ . __PS_BASE_URI__ . 'modules/lo/views/img/location-pin.png';
        $icon_posta = _PS_BASE_URL_SSL_ . __PS_BASE_URI__ . 'modules/lo/views/img/location-pin-posta.png';

        $sql = "
            SELECT
                dp.*,
                CASE dp_tip
                  WHEN 0 THEN '" . $icon_posta . "'
                  WHEN 1 THEN '" . $icon_pp . "'
                  ELSE ''
              END as icon,
    ...
    public function hookDisplayBeforeCarrier($params)
    {
        ...
        $servicii_nationale = json_decode(Configuration::get('LO_NATIONAL_FIELDS'), true);
            ...
            if ($serviciu['serviciuid'] == self::$lockers_service_id) {
                if ($serviciu['active']) {
                    $this->context->smarty->assign(array(
                        ...
                        'id_locker' => self::getSelectedLocker($this->context->cart->id)
                    ));
    ...
    public function getOrderShippingCost($params, $shipping_cost)
    {
        ...
        if (!empty($servicii_nationale)) {
            foreach ($servicii_nationale as $cf) {
                if ($cf['carrier'] == $carrier_reference && $cf['active'] == 1 && $cf['deleted'] == 0) {
                    $shipping_cost = $this->EstimeazaPret($params, $cf['serviciuid']);

                    if (!empty($cf['freeamount']) && $ramburs_ron >= $cf['freeamount']) {
                        $shipping_cost = 0;
                    }
                    if (!empty($cf['fixedprice']) && $cf['fixedprice'] > 0) {
                        $shipping_cost = (float)$cf['fixedprice'];
                    }

                    if ((float)$cf['modify_amount']) {
                        if ($cf['modify_how'] == 'amount') {
                            $diff = (float)$cf['modify_amount'];
                        } else {
                            $diff = (float)$shipping_cost * ((float)$cf['modify_amount'] / 100);
                        }
                    ...
                    $this->ship_cost[$this->id_carrier][$cart->nbProducts()] = (float)$shipping_cost;
                ...
                if ($carrier->external_module_name == $this->name) {
                    ...
                    if ($id_service == self::$lockers_service_id && $id_locker = self::getSelectedLocker((int)$order->id_cart)) {
                        ...
                        if ($params['newOrderStatus']->id == (int)Configuration::get('PS_OS_CANCELED')) {
                            ...
                            $lo->plus_expectedin($id_locker, $id_order);
                        }
                    }
                }
            }
            ...
            if (Validate::isLoadedObject($order)) {
                ...
                if ($id_carrier) {
                    ...
                    if ($carrier->external_module_name == $this->name && (!(int)Configuration::get('LO_STATUS') || (int)Configuration::get('LO_STATUS') == $order->getCurrentState())) {
                        $services = json_decode(Configuration::get('LO_NATIONAL_FIELDS'), true);
                        ...
                        $this->context->smarty->assign(array(
                            ...
                            'f_request_awb' => base64_encode(json_encode($f_request_awb)),
    ...
    {
        $carriers = json_decode(Configuration::get('LO_NATIONAL_FIELDS'));
    ...
    public function EstimeazaPret($params, $serviciuid)
    {
        $address = new Address($params->id_address_delivery);
        $cart = new Cart($params->id);
        //$currency = new Currency($cart->id_currency);

        $id_locker = self::getSelectedLocker($cart->id);
        ...
        $f_request_awb['serviciuid'] = $serviciuid;
        $f_request_awb['request_mpod'] = false;

        $national_fields = json_decode(Configuration::get('LO_NATIONAL_FIELDS'), true);

        if (isset($serviciuid) && (int)$serviciuid == self::$lockers_service_id && !$id_locker) {
            foreach ($national_fields as $carrier) {
                if ($carrier['serviciuid'] == $serviciuid && isset($carrier['flatprice']) && (float)$carrier['flatprice']) {
        ...
        if ($serviciuid != self::$lockers_service_id) {
            $f_request_awb['shipTOaddress'] = array(
                'address1' => $address->address1,
                'address2' => $address->address2,
                'city' => $address->city,
        ...
        if ($serviciuid == self::$lockers_service_id) {
            ...
            $raspuns = $lo->EstimeazaPretSmartlocker($f_request_awb, $id_locker, $cart->id);
        } else {
            $raspuns = $lo->EstimeazaPret($f_request_awb);
        }
