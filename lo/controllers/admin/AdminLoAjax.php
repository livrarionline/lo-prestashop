    ...
    public function initContent()
    {
        ...
        if (Tools::getValue('action') == 'getAwb') {
            ...
            if ((int)$serviciu[0] != $lo_module::$lockers_service_id) {
                ...
                if ($reservation->status == 'success') {
                    ...
                    if (isset($reservation->message)) {
                        ...
                        $error = $this->trans('We could not generate a reservation ID');
                    }
            ...
            if ($response_awb) {
                if (isset($response_awb->f_awb_collection)) {
                    $data = array(
                        ...
                        'serviciu' => $serviciu[2],
                        'f_token' => $response_awb->f_token,
                        'generated_awb_price' => $response_awb->generated_awb_price
                    );
                ...
                die(json_encode(array(
                    'success' => false,
                    'error' => $this->trans('No response from server. Please try again later.'),
                )));
            }
            ...
            if (isset($response_tracking->status) && $response_tracking->status == 'error') {
                $response = '<h3>' . sprintf($this->trans('Tracking AWB %s'), $awb) . '</h3>';
            ...
            if (isset($response_cancel->status) && $response_cancel->status == 'error') {
                $response = '<h3>' . sprintf($this->trans('Cancel AWB %s'), $awb) . '</h3>';
                ...
                if (isset($response_cancel->message) && $response_cancel->message) {
                    ...
                    $message = $this->trans('AWB was canceled.');
                }
            ...
            if (isset($response_cancel->status) && $response_cancel->status == 'error') {
                $response = '<h3>' . sprintf($this->trans('Return AWB %s'), $awb) . '</h3>';
                $response .= '<div>' . $response_cancel->message . '</div>';
                die(json_encode(array(
                    'success' => false,
                    'response' => $response,
                ...
                if (isset($response_cancel->message) && $response_cancel->message) {
                    ...
                    $message = sprintf($this->trans('Request for AWB return was made. Return awb: %s'),
