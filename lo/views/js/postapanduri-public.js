...
var map;
function plotMarkers(m, only_selected = false) {
    if (m) {
		map.removeMarkers();
		var markers_data = [];
        m.forEach(function (marker) {
          if(!only_selected || marker.dp_id == only_selected) {
            var temperatura = '';
            ...
            markers_data.push({
              id : marker.dp_id,
                ...
                infoWindow: {
                  content: '<div class="lo-map__infowindow infowindow">\
		...
		map.markers.forEach(function(value,index) {
			map.markers[index].addListener('click', function(e) {
				if (typeof window.disableMapMarkerClick !== 'undefined' && window.disableMapMarkerClick) {
					return;
				}
				window.disableMapMarkerClick = true;
				LoCheckSelectedLocker(map.markers[index].id);
				window.disableMapMarkerClick = false;
			});
