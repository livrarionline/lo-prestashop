...
class LO1
{
        ...
        if ($rows) {
            foreach ($rows as $row) {
                ...
                $delivery_points[] = array(
                    ...
                    'tip' => ($row->dp_tip == 0 ? 'Pachetomat' : 'Punct de ridicare'),
                    'orar' => $row->orar,
    ...
    // SMARTLOCKER UPDATE PUSH
    private function run_lockers_update_push()
    {
        $posted_json = file_get_contents('php://input');
        $lockers_post = $this->decrypt_ISSN($posted_json);

        if (empty($lockers_post)) {
            throw new \Exception('No data sent for Smartlocker update');
        }

        if (!empty($lockers_post->update) && !empty($lockers_post->f_stamp)) {
            $check_sql = "SELECT last_update FROM `" . _DB_PREFIX_ . "lo_locker_push`";
            $result = \Db::getInstance()->getValue($check_sql);
            ...
            $last_update_db = date('Y-m-d H:i:s');
            $lockers_data = $this->GetPachetomatePR(array('f_action' => 10, 'f_stamp' => $last_update));

            if (!empty($lockers_data)) {
                if (!empty($lockers_data->dulap)) {
                    foreach ($lockers_data->dulap as $delivery_point) {
                        \Db::getInstance()->insert(
                            "lo_delivery_points",
                            array(
                                ...
                                'termosensibil' => $delivery_point->termosensibil,
                                'img_indicatii' => $delivery_point->img_indicatii,
                                'img_pachetomat' => $delivery_point->img_pachetomat,
                ...
                if (!empty($lockers_data->zile2dulap)) {
                    foreach ($lockers_data->zile2dulap as $program) {
                        \Db::getInstance()->insert(
                            "lo_dp_program",
                            array(
                ...
                if (!empty($lockers_data->exceptii_zile)) {
                    foreach ($lockers_data->exceptii_zile as $exceptie) {
                        \Db::getInstance()->insert(
    ...
    private function run_lockers_update()
    {
        ...
        if (\Tools::getValue('force_lockers')) {
            ...
            $lockers_data = json_decode(json_decode($lockers_data), true);
        }
    ...
    private function run_issn()
    {
        ...
        $this->module->logissn('Decrypted ISSN: ' . json_encode($issn));
